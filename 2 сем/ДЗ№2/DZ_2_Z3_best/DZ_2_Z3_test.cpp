...
int main()
{
	...
	char equation[50], fixed_equation[50];
	float res;
	char cmd[20];
	int key, key2;
	//ввод
	do {
		...
		if (key != 3)
		{
			//исправление уравнения в случае необходимости
			strcpy(fixed_equation, equation);
			fix_equation(fixed_equation);
			if (strcmp(equation, fixed_equation) != 0)
				printf("Измененное уравнение: %s\n", fixed_equation);
			//построение дерева
			root = make_tree(fixed_equation);
			//вывод дерева
			//вывод дерева
...
void fix_equation(static char *equation)	//приведение уравнения к удобночитаемому виду в случае необходимости
{
	char *c;
	if (equation[0] == '+'  && equation[1] == '(')
		strcpy(equation, equation + 1);
	while (c = strstr(equation, "(+("))
		strcpy(c + 1, c + 2);
	...
	{
		int len = strlen(equation);
		for (int i = len; i > c - equation + 1; i--)
			equation[i] = equation[i - 1];
		equation[c - equation + 1] = '0';
		equation[len + 1] = '\0';
	}
	c = equation;
	while (*c)
	{
		if (strchr("+-", *c) && isalpha(*(c + 1)))
		{
			if (c - equation == 0 || *(c - 1) == '(')
			{
				int len = strlen(equation);
				for (int i = len; i > c - equation; i--)
					equation[i] = equation[i - 1];
				equation[c - equation] = '0';
				equation[len + 1] = '\0';
				c++;
			}
		}
...
{
	int cur, bracket = 0, k = 0, pr_key = 0;
	static int lvl = 0;
	...
	} while (pr_key != 1 && cur != left);	//выход если найден подходящий оператор или встречен конец уравнения

	node *new_node = new node;
	new_node->left = nullptr;
	new_node->right = nullptr;
	if ((k == 0 && pr_key == 0) || equation[k - 1] == '(')	//если не найден оператор или перед оператором (
	{
